...
import type { BundlerState, SandpackClient, SandpackMessage } from '@codesandbox/sandpack-client';
import { useEffect, useState } from 'react';

function getTranspiledCode(client: SandpackClient): string | null {
  if (!client) return null;
  
  const bundlerState = client['bundlerState' as keyof SandpackClient] as BundlerState;
