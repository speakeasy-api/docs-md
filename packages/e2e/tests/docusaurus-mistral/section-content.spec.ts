...
test.describe('SectionContent', () => {
  test('should expand content when expandable button is clicked', async ({ page }) => {
    await page.goto('mistral/api/endpoint/ocr#operation-ocr_v1_ocr_post+request+document');
    await page.waitForLoadState('networkidle');
    const expandFileChunkButton = page.getByTestId('expandable-cell-button-operation-ocr_v1_ocr_post+request+document+filechunk');
